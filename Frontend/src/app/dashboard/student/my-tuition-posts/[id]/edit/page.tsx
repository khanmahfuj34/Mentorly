...
import { getSingleTuitionRequest, updateTuitionRequest } from "@/src/services/student/tuition.service"
...
export default function EditTuitionPostPage({ params }: EditPageProps) {
  ...
  const [isLoadingTuition, setIsLoadingTuition] = useState(true)
  ...
  useEffect(() => {
    const fetchProfileAndTuition = async () => {
      try {
        ...
        if (comp.isComplete) {
          ...
          const tuitionRes = await getSingleTuitionRequest(id)
          if (tuitionRes?.data) {
            const tuitionData = tuitionRes.data
            setClassLevel(tuitionData.classLevel || "")
            setSubject(tuitionData.subject || "")
            setMedium(tuitionData.medium || "")
            setGenderPreference(tuitionData.genderPreference || "Any")
            setDistrict(tuitionData.district || "")
            setArea(tuitionData.area || "")
            setSalary(tuitionData.salary ? String(tuitionData.salary) : "")
            setDaysPerWeek(tuitionData.daysPerWeek ? String(tuitionData.daysPerWeek) : "")

            // Parse address and description
            const rawDescription = tuitionData.description || ""
            let parsedAddress = ""
            let parsedDescription = rawDescription

            if (rawDescription.startsWith("Address: ")) {
              const parts = rawDescription.split("\n\n")
              parsedAddress = parts[0].replace("Address: ", "").trim()
              parsedDescription = parts.slice(1).join("\n\n").replace(/^Description:\s*/, "").trim()
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await updateTuitionRequest(id, payload)
      
      toast.success(res?.message || "Tuition updated successfully.")
      router.push("/dashboard/student/my-tuition-posts")
      
    } catch (error) {
      const err = error as { response?: { data?: { message?: string } }; message?: string }
      const errorMessage =
        err?.response?.data?.message || err?.message || "Failed to update tuition request."
  ...
  if (isLoadingTuition) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-on-surface-variant font-body-md">Loading tuition post details...</p>
      </div>
    )
  }
    ...
    <div className="max-w-7xl mx-auto px-6 py-10 md:px-10">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-on-surface-variant mb-3">
          ...
          <span className="text-on-surface font-semibold">Edit Tuition Post</span>
        </div>
        <h2 className="text-headline-lg font-bold text-on-surface mb-2">Edit Tuition Request</h2>
        <p className="text-on-surface-variant font-body-md">
          Modify your tuition requirements to update active listings for tutors.
        </p>
      </div>
